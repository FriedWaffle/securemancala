    ...
    class Chat extends DB
    {
        ...
        function read($lobby_id)
        {
            try
            {
                ...
                {
                    $arr[$i] = array('player' => $row['username'], 'msg' => $row['message'], 'chat_id' => $row['chat_id'], 'id' => $row['id'], 'ready' => $row['ready']);
                    $i++;
                }
